...
#+BEGIN_SRC emacs-lisp
  ...
  (setq-default mode-line-format '("%e" mode-line-front-space
                                   ...
                                   (vc-mode vc-mode)
                                   "  " minions-mode-line-modes mode-line-misc-info mode-line-end-spaces flymake-mode-line-format))

  ;; Increase height
  (custom-set-faces
   '(mode-line ((t (:height 150))))
   '(mode-line-inactive ((t (:height 150)))))

  ;; Change buffer font color if it is modified
  (defun update-buffer-name-color ()
    "Update buffer name color based on buffer status."
    (let ((color (cond
                  ((or buffer-read-only (string-match-p "\\`\\*" (buffer-name))) "#ff6ac1") ; Color for read-only or ephemeral buffers
                  ((buffer-modified-p) "#ff9f43") ; Color for modified buffers
                  (t "#5af78e")))) ; Default color
      (set-face-attribute 'mode-line-buffer-id nil :foreground color)))

  (add-hook 'after-save-hook 'update-buffer-name-color)
  (add-hook 'post-command-hook 'update-buffer-name-color)

  ;; Initial color update for already opened buffers
  (dolist (buf (buffer-list))
    (with-current-buffer buf
      (update-buffer-name-color)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package evil
    ...
    ;; Mode line
    (setq evil-normal-state-tag   (propertize " NORMAL " 'face '((:background "#5af78e" :foreground "black")))
          evil-emacs-state-tag    (propertize " EMACS  " 'face '((:background "#ff6ac1" :foreground "black")))
          evil-insert-state-tag   (propertize " INSERT " 'face '((:background "#ff9f43" :foreground "black")))
          evil-replace-state-tag  (propertize " REPLACE " 'face '((:background "#ff9f43" :foreground "black")))
          evil-motion-state-tag   (propertize " MOTION " 'face '((:background "#f3f99d" :foreground "black")))
          evil-visual-state-tag   (propertize " VISUAL " 'face '((:background "#9aedfe" :foreground "black")))
          evil-operator-state-tag (propertize " OPERATE " 'face '((:background "#57c7ff" :foreground "black"))))
