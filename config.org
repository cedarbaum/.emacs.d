...
Install Emacs via [[https://github.com/d12frosted/homebrew-emacs-plus][emacs-plus]]:

#+BEGIN_SRC shell :tangle no
  brew tap d12frosted/emacs-plus
  brew install emacs-plus@30 --with-xwidgets --with-mailutils --with-imagemagick
#+END_SRC

Copy =Emacs.app= to =/Applications= so it works with Spotlight, Dock, and cmd-tab. Re-run this after =brew upgrade emacs-plus@30=.

#+BEGIN_SRC shell :tangle no
  cp -r /opt/homebrew/opt/emacs-plus@30/Emacs.app /Applications/
#+END_SRC

Optionally, run as a background daemon:
...
The file =~/.authinfo.json= (or its GPG-encrypted variant =~/.authinfo.json.gpg=)
stores secrets as a JSON array of objects. Each object is matched by =auth-source-search=
using standard keys like =:host=. Custom keys (e.g. =:api_key=) are extracted with =plist-get=.
...
A skeleton file is provided at =~/.authinfo.json= -- replace placeholder values with real secrets.
...
Enable auto revert mode. Disable vc-info re-checking on revert since magit handles git status.
...
#+BEGIN_SRC emacs-lisp
  ...
  (when-let ((font (get-preferred-font 18)))
...
Log native compilation warnings to =*Warnings*= but don't pop up the buffer.
...
Use [[https://github.com/emacsmirror/gcmh][gcmh]] (Garbage Collection Magic Hack) to defer GC to idle time. The =early-init.el= sets =gc-cons-threshold= to =most-positive-fixnum= during startup; gcmh takes over afterward, using a high threshold while typing and triggering GC when idle.
...
Optimize bidirectional text rendering. If you only use left-to-right languages, this avoids unnecessary reordering computation on every line.
...
Automatically handle files with very long lines (e.g., minified JS/CSS) by disabling expensive features.

#+BEGIN_SRC emacs-lisp
  (global-so-long-mode 1)
#+END_SRC

Reduce UI updates for smoother editing.

#+BEGIN_SRC emacs-lisp
  (setq idle-update-delay 1.0)                    ;; update screen less aggressively
  (setq fast-but-imprecise-scrolling t)            ;; skip fontification while scrolling fast
  (setq-default cursor-in-non-selected-windows nil) ;; don't render cursor in unfocused windows
#+END_SRC
...
Built-in LSP integration. Disable event logging for performance and auto-shutdown idle servers.
...
[[https://github.com/manzaltu/claude-code-ide.el][claude-code-ide.el]] provides deep integration with Claude Code CLI via MCP. Claude can query LSP, tree-sitter, and project state. Code suggestions are reviewed via ediff.
...
#+BEGIN_SRC emacs-lisp
  (use-package exec-path-from-shell
    ...
    :config
    ;; Use login shell only (not interactive) for faster startup
...
Display possible keybindings after an incomplete prefix. Built-in as of Emacs 30.
...
#+BEGIN_SRC emacs-lisp
  (use-package wttrin
    :config
    (setq wttrin-default-locations '("New York, NY" "Chicago, IL")))
#+END_SRC
